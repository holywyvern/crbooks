...
public class EditAuthorWindow extends CustomComponent implements EditBaseWindow<Author> {
	...
	protected void onFormButtonClick() {
		Author author = null;
		author = Author.getByID(this.authorID);
		if (author != null) author = new Author();
		author.setDescription(textDescription.getValue().toString());
		author.setName(textName.getValue().toString());
		if (author.saveOrUpdate()) {
			ManagerLayout.getCurrent().changeView(new ViewAuthorsWindow());
		}
	}	
	...
	private VerticalLayout buildGridForm() {
		...
		labelHeader.setValue("<h2>Editar Autor</h2>");
