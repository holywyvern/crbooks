...
public class EditOrderStateWindow extends CustomComponent  implements EditBaseWindow<OrderState> {
	...
	protected void onFormButtonClick() {
		OrderState state = OrderState.getByID(stateID);
	...
	private VerticalLayout buildGridFormPanel() {
		...
		labelFormHeader.setValue("<h2>Agregar un nuevo Estado</h2>");
	...
	private GridLayout buildGridFormContent() {
		...
		buttonForm.setCaption("Agregar Estado");
	...
	public void edit(OrderState item) {
		this.textDescription.setValue(item.getDescription());
		this.textName.setValue(item.getName());
		this.checkFinal.setValue(item.isFinal());
		this.stateID = item.getOrderStateID();
	}	
